...
def test_invalid_type_nested():
    with pytest.raises(TypeError) as exc_info:
        tomli_w_null.dumps({"bytearr": bytearray()})
...
def test_invalid_time():
    ...
    with pytest.raises(ValueError):
        tomli_w_null.dumps({"offset time": offset_time})


def test_negative_indent():
    with pytest.raises(ValueError):
        tomli_w_null.dumps({"k": "v"}, indent=-1)


def test_invalid_key__falsy():
    with pytest.raises(TypeError) as exc_info:
        tomli_w_null.dumps({None: "v"})  # type: ignore[dict-item]
    assert (
        str(exc_info.value)
...
def test_invalid_key__truthy():
    with pytest.raises(TypeError) as exc_info:
        tomli_w_null.dumps({2: "v"})  # type: ignore[dict-item]
    assert (
        str(exc_info.value)
